...
namespace GSpawn
{
    public class ObjectExtrudeGizmo : PluginGizmo
    {
        ...
        protected override void doOnSceneGUI()
        {
            ...
            if (padding.x <= -_boxSize.x) padding.x = -_boxSize.x + 1e-2f;
            if (padding.y <= -_boxSize.y) padding.y = -_boxSize.y + 1e-2f;
            if (padding.z <= -_boxSize.z) padding.z = -_boxSize.z + 1e-2f;
            sharedSettings.padding = padding;
